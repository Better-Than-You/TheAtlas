...
module.exports = {
  ...
  start: async (Atlas, m, { inputCMD, text, doReact, prefix, pushName }) => {
    switch (inputCMD) {
      ...
      case "search":
        ...
        await Atlas.sendMessage(
          ...
          {
            image: {
              url: "https://i.imgur.com/KvhzUWU.jpg",
            },
            caption: resText,
          },
          { quoted: m }
    ...
    if(!text){
        doReact('❌');
        return m.reply('Please provide a valid search term\n\nExample: *-define <the search term>*')
    }            axios.get(`https://api.urbandictionary.com/v0/define?term=${text.toLowerCase()}`).then(res=>{
